...
type LevelDBBookRepo struct {
	cache map[utils.ISBN]database.Timed[Book]
	mu    sync.RWMutex
	db    *leveldb.DB
}

func NewInMemoryBookRepo(dbPath string) (BookRepo, error) {
	db, err := leveldb.OpenFile(dbPath, nil)
	if err != nil {
		return nil, err
	}
	repo := &LevelDBBookRepo{cache: make(map[utils.ISBN]database.Timed[Book]), db: db}
...
func (repo *LevelDBBookRepo) EditaLivro(book Book) (api_cad.Status, error) {
	exists, err := repo.db.Has([]byte(book.Isbn), nil)
	if err != nil {
...
func (repo *LevelDBBookRepo) RemoveLivro(id utils.ISBN) (api_cad.Status, error) {
...
func (repo *LevelDBBookRepo) ObtemLivro(id utils.ISBN) (Book, error) {
...
func (repo *LevelDBBookRepo) ObtemTodosLivros() ([]Book, error) {
	var books []Book
...
func (repo *LevelDBBookRepo) getFromCache(isbn utils.ISBN) (Book, bool) {
	repo.mu.RLock()
