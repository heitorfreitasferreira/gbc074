...
import (
	...
	"github.com/rpc-mqtt-library-manager/crud-terminal-client/internal/entity"
)

func NewReadUserScreen() (entity.User, error) {
	p := tea.NewProgram(NewReadUserModel())
	if model, err := p.Run(); err != nil {
		return entity.User{}, err
	} else {
		return model.(ReadUserModel).readUser, nil
	}
}
...
func NewReadUserModel() ReadUserModel {
	m := ReadUserModel{
...
func (c ReadUserModel) Init() tea.Cmd {
	return textinput.Blink
}

func (c ReadUserModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "ctrl+c", "esc":
...
func (m *ReadUserModel) updateInputs(msg tea.Msg) tea.Cmd {
...
func (m ReadUserModel) View() string {
